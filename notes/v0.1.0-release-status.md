...
**Windows build: PASSING** — pre-built wheel approach working with baseline x86-64

Both platforms verified in CI run `22280082450` on branch `prebuilt-wheels`.

## Next Steps

1. Merge `prebuilt-wheels` branch to `main`
2. Delete old v0.1.0 tag, create new tag, push
3. Test the Windows exe on desktop
4. Contribute `build-wheels.yml` upstream to anuga-community
