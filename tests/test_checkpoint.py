"""Tests for TASK-1919 — checkpoint write gate on the Batch / no-resume path.

TASK-1048 established there is no checkpoint-resume on AWS Batch (spot loss
accepted).  The pickles are pure scratch that fill the root volume linearly.
The gate: when AWS_BATCH_JOB_ID is present, checkpoints are disabled unless
RUN_ANUGA_CHECKPOINTS=on overrides.  Locally (no AWS_BATCH_JOB_ID) checkpoints
remain enabled unless RUN_ANUGA_CHECKPOINTS=off overrides.
"""
